...
export async function GET() {
  ...
  const fonts = files
    ...
    .map((file) => {
      ...
      const firstFontEntry = zip
        .getEntries()
        .find((entry: ZipEntry) => entry.entryName.match(/\.(ttf|otf)$/));
      ...
      if (firstFontEntry) {
        ...
        if (fileBuffer) {
          ...
          const mimeType = firstFontEntry.entryName.endsWith(".ttf")
            ? "font/ttf"
            : "font/otf";
          ...
          let fontName = path
            .basename(
              firstFontEntry.entryName,
              path.extname(firstFontEntry.entryName),
            )
